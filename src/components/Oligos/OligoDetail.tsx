...
import { Oligo, Organism, User } from '../../types';
import { dataManager } from '../../services/dataManager';

interface OligoDetailProps {
  oligo: Oligo | null;
  open: boolean;
  onClose: () => void;
  onSave?: (updatedOligo: Oligo) => void;
}

const OligoDetail: React.FC<OligoDetailProps> = ({ oligo, open, onClose, onSave }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [editData, setEditData] = useState<Oligo | null>(null);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to save oligo');
    } finally {
      setSaving(false);
    }
  };
  ...
  const displayData = editData || oligo;

  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6">
          Oligo Details - {displayData.refid}
        </Typography>
        <Box>
          {!isEditing && (
      ...
      <DialogContent dividers>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
              ...
              {isEditing ? (
                <>
                  <TextField
                    ...
                    onChange={(e) => setEditData(prev => prev ? { ...prev, length: Number(e.target.value) } : null)}
                  />
                  <TextField
                    label="Melting Temp"
                    type="number"
                    inputProps={{ step: "0.1" }}
                    value={editData?.meltingtemp || ''}
                    onChange={(e) => setEditData(prev => prev ? { ...prev, meltingtemp: Number(e.target.value) } : null)}
                  />
                  <TextField
                    label="GC %"
                    type="number"
                    inputProps={{ step: "0.1" }}
                    value={editData?.gc || ''}
                    onChange={(e) => setEditData(prev => prev ? { ...prev, gc: Number(e.target.value) } : null)}
                  />
                </>
              ) : (
                <>
                  <Box>
                    ...
                    <Typography variant="body1">{displayData.length || 'N/A'}</Typography>
                  </Box>
                  <Box>
                    <Typography variant="subtitle2" color="text.secondary">Melting Temp (°C)</Typography>
                    <Typography variant="body1">
                      {displayData.meltingtemp ? displayData.meltingtemp.toFixed(1) : 'N/A'}
                    </Typography>
                  </Box>
                  <Box>
                    <Typography variant="subtitle2" color="text.secondary">GC %</Typography>
                    <Typography variant="body1">
                      {displayData.gc ? displayData.gc.toFixed(1) : 'N/A'}
                    </Typography>
                  </Box>
                </>
              )}
          ...
          {/* Right Column */}
          <Box>
            <Typography variant="h6" gutterBottom>Sequence Information</Typography>
            ...
            {isEditing ? (
              <TextField
                ...
                rows={4}
                value={editData?.seq || ''}
                onChange={(e) => setEditData(prev => prev ? { ...prev, seq: e.target.value } : null)}
              ...
              <Box sx={{ mb: 2 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                  ...
                  {displayData.seq && (
                    <IconButton size="small" onClick={() => copyToClipboard(displayData.seq)}>
                      <FileCopy fontSize="small" />
                    </IconButton>
                  )}
                </Box>
            ...
            {isEditing ? (
              <TextField
                fullWidth
                label="Useful Sequence"
                multiline
                rows={3}
                value={editData?.usefulseq || ''}
                onChange={(e) => setEditData(prev => prev ? { ...prev, usefulseq: e.target.value } : null)}
                sx={{ mb: 2, fontFamily: 'monospace' }}
              />
            ) : (
              <Box sx={{ mb: 2 }}>
                <Typography variant="subtitle2" color="text.secondary">Useful Sequence</Typography>
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                  <Typography 
                    variant="body2" 
                    sx={{ 
                  ...
                  >
                    {displayData.usefulseq || 'N/A'}
                  </Typography>
                  {displayData.usefulseq && (
                    <IconButton size="small" onClick={() => copyToClipboard(displayData.usefulseq)}>
                      <FileCopy fontSize="small" />
                    </IconButton>
                  )}
                </Box>
